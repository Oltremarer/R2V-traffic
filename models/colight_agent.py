...
class CoLightAgent(Agent):
    ...
    def load_network(self, file_name, file_path=None):
        ...
        self.q_network = build_colight_model_with_weights(
            self.build_network,
    ...
    def load_network_bar(self, file_name, file_path=None):
        if file_path is None:
            file_path = self.dic_path["PATH_TO_MODEL"]
        self.q_network_bar = build_colight_model_with_weights(
            self.build_network,
            os.path.join(file_path, "%s.h5" % file_name),
        )
...
def colight_custom_objects():
    return {"RepeatVector3D": RepeatVector3D}
...
def load_colight_model(path):
    enable_trusted_lambda_deserialization()
    kwargs = {
        "custom_objects": colight_custom_objects(),
        "compile": False,
    }
    try:
...
def build_colight_model_with_weights(build_model, path):
    network = build_model()
    try:
        network.load_weights(path)
    except ValueError:
        network.load_weights(path, by_name=True)
    return network
