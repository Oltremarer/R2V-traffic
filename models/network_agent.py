...
class NetworkAgent(Agent):
    ...
    def load_network(self, file_name, file_path=None):
        ...
        self.q_network = load_network_model(os.path.join(file_path, "%s.h5" % file_name))
        self._compile_network(self.q_network)
    ...
    def load_network_bar(self, file_name, file_path=None):
        if file_path is None:
            file_path = self.dic_path["PATH_TO_MODEL"]
        self.q_network_bar = load_network_model(os.path.join(file_path, "%s.h5" % file_name))
        self._compile_network(self.q_network_bar)
        print("succeed in loading model %s" % file_name)
    ...
    def build_network_from_copy(self, network_copy):
        ...
        enable_trusted_lambda_deserialization()
        network = model_from_json(network_structure, custom_objects=network_custom_objects())
        network.set_weights(network_weights)
        return self._compile_network(network)

    def _compile_network(self, network):
        network.compile(
            optimizer=Adam(learning_rate=self.dic_agent_conf["LEARNING_RATE"]),
            loss=self.dic_agent_conf["LOSS_FUNCTION"],
            run_eagerly=os.environ.get("PAPER_FINAL_REFERENCE_RUN") == "1",
        )
        return network
...
def network_custom_objects():
    return {
        "Selector": Selector,
        "slice_tensor": slice_tensor,
        "relation": relation,
        "sum_axis_2": sum_axis_2,
    }


def enable_trusted_lambda_deserialization():
    if hasattr(keras, "config") and hasattr(keras.config, "enable_unsafe_deserialization"):
        keras.config.enable_unsafe_deserialization()


def load_network_model(path):
    enable_trusted_lambda_deserialization()
    kwargs = {
        "custom_objects": network_custom_objects(),
        "compile": False,
    }
    try:
        return load_model(path, safe_mode=False, **kwargs)
    except TypeError:
