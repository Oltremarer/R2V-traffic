...
def _apply_paper_final_reference_override(dic_traffic_env_conf, dic_path):
    if os.environ.get("PAPER_FINAL_REFERENCE_RUN") != "1":
        return False
    out_dir = os.environ.get("PAPER_FINAL_REFERENCE_OUT_DIR", "")
    seed_id = os.environ.get("PAPER_FINAL_REFERENCE_SEED_ID", "")
    model_dir = os.environ.get("PAPER_FINAL_REFERENCE_MODEL_DIR", "")
    if not out_dir.startswith("records/paper_final/"):
        raise ValueError("PAPER_FINAL_REFERENCE_OUT_DIR must be under records/paper_final")
    if ".." in out_dir.split(os.sep):
        raise ValueError("PAPER_FINAL_REFERENCE_OUT_DIR must not contain '..'")
    if not model_dir:
        raise ValueError("PAPER_FINAL_REFERENCE_MODEL_DIR is required")
    seed = int(seed_id)
    dic_path["PATH_TO_WORK_DIRECTORY"] = out_dir
    dic_path["PATH_TO_MODEL"] = model_dir
    dic_traffic_env_conf["CITYFLOW_SEED"] = seed
    dic_traffic_env_conf["REFERENCE_POLICY_SEED"] = seed
    return True
...
def pipeline_wrapper(dic_agent_conf, dic_traffic_env_conf, dic_path, roadnet, trafficflow):
    ...
    all_travel_time = []
    for i in range(1):
        exact_paper_final_path = _apply_paper_final_reference_override(dic_traffic_env_conf, dic_path)
        if not exact_paper_final_path:
